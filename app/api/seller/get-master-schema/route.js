...
export async function GET(req) {
  ...
  try {
    ...
    if (!schemas.length) {
      return new Response(JSON.stringify({ message: "No schemas found" }), { status: 404 });
    }
    ...
    return new Response(JSON.stringify({ message: "Failed to fetch master schemas" }), { status: 500 });
  } finally {
